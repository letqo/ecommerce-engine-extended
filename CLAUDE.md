...
- Supplier platform expansion (Printful/Gelato/BigBuy/WooBridge adapters, per-store supplier enablement, compliance profiles) — built overnight 2026-08-03 on a branch, merged and deployed to production the same day. See `OVERNIGHT_BUILD_NOTES.md` for judgment calls; most of its "explicit follow-ups" list is now stale (Gelato print files and the generalized product-import UI are done, see below) — check current branch/PR history before trusting that doc's follow-up list over this section.
...
- **Checkout country dropdown — merged to `main` 2026-08-03** (`39c2cfe`, formerly branch `feature/checkout-country-dropdown`): free-text country input → a `<select>` built from `Store.targetMarkets`, localized via `Intl.DisplayNames`. Live and deployed.
- **Product import generalized to all configurable suppliers 2026-08-03/04** (`09ba4b5`, merged `29debd3`) — the admin Import Products page and tracking sync are no longer CJ/AliExpress-only; this closed overnight-build follow-up #7.
- **Gelato fully fixed and print-file-capable as of 2026-08-04**: catalog list/search/product responses now match the real API shape (`0387aa6`), search reads the merchant's own store products instead of the raw catalog (`6657a71`), and print-file support was added (`4d36338`) — closed overnight-build follow-up #1 ("Gelato is not usable end-to-end without this"). All merged to `main` and deployed. Old branches `feature/gelato-catalog-picker`, `feature/gelato-print-files`, `fix/gelato-real-api-shape`, `fix/gelato-store-products-api` are now fully merged (0 commits ahead of `main`) and can be deleted whenever convenient.
- **Webhook routes for Printful and WooBridge added 2026-08-04** — `POST /api/webhooks/printful/:storeId` and `/api/webhooks/woo/:storeId` (`backend/src/routes/webhooks/printful.ts`, `woo.ts`), mounted in `app.ts` ahead of the global JSON parser, `:storeId` scoping the lookup since these routes never go through `resolveStore`. Closed overnight-build follow-up #4. The admin Integrations page now shows each supplier's webhook URL (copy button) once expanded, so the store owner can actually paste it into Printful/WooCommerce's dashboard. **Found and fixed in the same pass: the CJ webhook route (`/api/webhooks/cj`) had no body parser mounted at all** — `req.body` was always `{}`, so every CJ webhook silently did nothing since it was first written (the handler's `req.body && typeof req.body === 'object' ? req.body : {}` fallback masked it — no crash, no error, just a permanently-empty payload). Fixed by adding `express.json()` to its mount alongside the two new routes. **Also hit and fixed a genuine Express gotcha while building this**: a `Router` mounted via `app.use('/path/:param', router)` does NOT get `:param` in its own `req.params` unless created with `Router({ mergeParams: true })` — verified this with a standalone repro before shipping, since the failure mode is silent (`req.params.storeId` is just `undefined`, no error) and would have made both new webhook routes permanently no-op exactly like the CJ bug above.
- **Still open, per the overnight-build follow-ups list**: BigBuy shipping-coverage check still fails open on a `TODO(real-docs-needed)` (#3); no adapter contract tests, no automated tests in the repo at all (#6); compliance field labels aren't i18n'd (#5).
